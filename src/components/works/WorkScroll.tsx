    ...
    <section>
      ...
      >
        {items.map((item, index) => (
          <WorkScrollItem key={index} item={item} />
        ))}
      </div>
      <div
        css={css`
...
function WorkScrollItem({ item }: { item: WorkGetResponse }) {
  return (
    <div
      className="work-scroll-item"
      css={css`
        position: relative;
    ...
    >
      ...
      >
        <Image
          src={item.images && item.images[0] ? item.images[0].url : "/images/no-image.png"}
          fill
          alt={item.title}
          css={css`
            object-fit: contain;
