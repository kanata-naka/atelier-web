...
export default ({ tagsInfo }) => {
  ...
  // タグの最大件数
  const maxCount = Object.keys(tagsInfo).reduce((a, b) => Math.max(a, tagsInfo[b]), 1)
    ...
    <div className={`tags-info ${!collasped ? "expanded" : ""}`}>
      <ul className="tag-list">
        {Object.keys(tagsInfo).map((tag, index) => {
          ...
          return <TagInfo key={index} tag={tag} count={tagsInfo[tag]} rate={rate} />
        })}
      </ul>
      <div className="tags-info-foreground" />
      <div className="tags-info-slide-button">
        <a href="#" className="tags-info-slide-button__link" onClick={handleClickToggleButton}>
          {
            collasped ? <i className="fas fa-chevron-down"></i>
      ...
      <Link href={`?tag=${tag}`}>
        <a className="tag-list-item__link">
          <span className="tag-name" style={{
            ...
            color: `rgba(${Math.max(0, 88 - (80*rate))},${Math.max(0, 88 - (80*rate))},${Math.max(0, 221 - (140*rate))},1)`
          }}>{tag}</span>
